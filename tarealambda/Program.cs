...
while (op <= 6) // con este while hare funcionar el menu, hasta que el usuario ponga una numero = o > que 7;
{
    ...
    switch (op)
    {
        //case 1:// Inicializacion de vectores // asi lo teniamos en el codigo
        ...
        case 1:// Inicializacion de vectores // implementacion lambda

            Console.WriteLine();
            Array.ForEach(Nombres, n => n = "");
            Array.ForEach(Cedulas, c => c = "");
            Array.ForEach(promedio, p => p = 0);
            Array.ForEach(Condicion, con => con = "");
            Console.WriteLine("Vectores iniciados Exitosamente");
            Console.WriteLine("Vectores iniciados Exitosamente");
            break;
        ...
        case 4:
            ...
            encontrado = Cedulas.Any(cedula => cedula.Equals(Identificacion, StringComparison.OrdinalIgnoreCase));

            if (encontrado)
            {
                Console.WriteLine($"Cedula {Identificacion} Encontrada");
                Console.WriteLine("Ponga el nombre del estudiante: ");
                Nombres[Array.FindIndex(Cedulas, c => c.Equals(Identificacion))] = Console.ReadLine();
                ...
                do
                {
                    try
                    {
                        promedio[Array.FindIndex(Cedulas, c => c.Equals(Identificacion))] = float.Parse(Console.ReadLine());

                        while (promedio[Array.FindIndex(Cedulas, c => c.Equals(Identificacion))] > 100 || promedio[Array.FindIndex(Cedulas, c => c.Equals(Identificacion))] < 0)
                        {
                            Console.WriteLine("El promedio no puede ser menor a 0 o mayor a 100, Digite el promedio del estudiante");
                            promedio[Array.FindIndex(Cedulas, c => c.Equals(Identificacion))] = float.Parse(Console.ReadLine());
                        }
                ...
                Condicion[Array.FindIndex(Cedulas, c => c.Equals(Identificacion))] = (promedio[Array.FindIndex(Cedulas, c => c.Equals(Identificacion))] >= 70) ? "Aprobado" : "Reprobado";
                Console.WriteLine();
            }
            else
            {
